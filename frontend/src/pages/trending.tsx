...
const Blogs = () => {
   ...
   const [trendblogs, settrendingBlogs] = useState<Blog[]>([]);
   ...
   useEffect(() => {
      ...
      const fetchTrendingBlogs = async () => {
         try {
            const response = await axios.get("http://localhost:3000/api/v1/blogs/trending", {
      ...
      const fetchUserInfo = async () => {
         try {
            const { data: { data: userInfo } } = await axios.get("http://localhost:3000/api/v1/user/info", {
               headers: { Authorization: `Bearer ${token}` },
            });
      ...
      const fetchProfiles = async () => {
         try {
            const { data: profileData } = await axios.get("http://localhost:3000/api/v1/blogs/profiles", {
               headers: { Authorization: `Bearer ${token}` },
            });
            const usernames = profileData.map((profile: { author: { username: string } }) => profile.author.username);
               ...
               {trendblogs.map((blog) => (
                  <div key={blog.id} className="border border-white p-6 rounded-md mb-6 bg-black">
                     ...
                     <p className="text-gray-300">
                        {expanded === blog.id ? blog.content : `${blog.content.slice(0, 100)}...`}
                     </p>
         ...
         {showMenu && (
            <div className="fixed top-0 right-0 w-64 h-full bg-black text-white p-4 z-50 shadow-lg">
               ...
               <ul className="space-y-4">
                  ...
                  <li className="hover:text-sky-400 transition-colors cursor-pointer">Create Post</li>
                  <li className="hover:text-sky-400 transition-colors cursor-pointer">Your Posts</li>
                  <li className="hover:text-sky-400 transition-colors cursor-pointer">Discover More</li>
               </ul>
            </div>
         )}
      </div>
